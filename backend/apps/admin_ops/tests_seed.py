...
class SeededApiIntegrationTests(APITestCase):
    ...
    def test_seeded_endpoints_return_realistic_member_facing_payloads(self):
        ...
        product_filter_config_response = self.client.get(reverse("product_filter_config"))
        product_search_response = self.client.get(reverse("product_search"), {"category": "gold-chain", "purity": "22K", "length": "18 inch"})
        ads_response = self.client.get(reverse("ads_overview"), {"placement": "dashboard_hero"})
        ...
        row_titles = [row["title"] for row in market_response.data["rows"]]
        self.assertIn("Browse Categories", row_titles)
        self.assertIn("Hero Spotlight", row_titles)
        ...
        self.assertTrue(any(category["slug"] == "gold-chain" for category in product_filter_config_response.data["categories"]))
        self.assertEqual(product_search_response.status_code, 200)
        self.assertGreaterEqual(product_search_response.data["count"], 1)
        self.assertTrue(any(item["attribute_value"] == "18 inch" for item in product_search_response.data["results"]))
        self.assertEqual(ads_response.status_code, 200)
        ...
        self.assertEqual(news_response.data["featured_news"]["title"], "GST update issued for bullion traders")
        self.assertTrue(news_response.data["featured_news"]["image_url"])
        self.assertTrue(any(item["image_url"] is None for item in news_response.data["items"]))
